...
class DiscordBot {
  ...
  initialize () {
    this.bot = new Discord.Client({
      ...
      owner: config.owner || '320674163515850754',
      ...
      this.bot.dispatcher.addInhibitor(msg => {
        if (msg.guild && !this.authorizedOwners.includes(msg.guild.ownerID)) {
          if (this.authorizedOwners.length === 0) {
            ...
            msg.reply(`Sorry, this server isn't authorized to use RoVer Plus.${msg.member.hasPermission(['MANAGE_GUILD']) ? ' The server owner needs to donate at <https://www.patreon.com/erynlynn>' : ''}`) // notify sender to donate only if they're an "admin"
          }
  ...
  ready () {
    ...
    this.bot.user.setActivity('Crescendo Verification Sistem', { type: "LISTENING" })
  }
